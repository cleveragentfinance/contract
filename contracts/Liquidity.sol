...
contract Liquidity is Ownable, ReentrancyGuard {
    ...
    function withdraw(uint256 _amount) public onlyOwner {
        ...
        IERC20(depositToken).approve(lpRouter, _amount);
        IUniswapV2Router01(lpRouter).removeLiquidity(token0, token1, _amount, 0, 0, address(this), block.timestamp);
        if(receiveToken != token0) {
        ...
        totalDepositedAmount -= totalDepositedAmount.mul(_amount).div(contractBalance);
    }
    ...
    function totalValueLocked() public view returns (uint256) {
        uint256 lockedAmount = IUniswapPair(depositToken).balanceOf(address(this));
        address token0 = IUniswapPair(depositToken).token0();
        address token1 = IUniswapPair(depositToken).token1();
        uint256 lpSupply = IUniswapPair(depositToken).totalSupply();
        uint256 share0 = IERC20(token0).balanceOf(depositToken).mul(lockedAmount).div(lpSupply);
        uint256 share1 = IERC20(token1).balanceOf(depositToken).mul(lockedAmount).div(lpSupply);
    ...
    function pendingReward() public view returns (uint256) {
        return totalValueLocked() - totalDepositedAmount;
    }
